%% Topic 1 Workshop Sheet, regression - BROKEN VERSION
%% load the data
...
n=length(x);
% regression formula
% a= (sum(x^2)*sum(y)-(sum(x)*sum(x.*y)))/( n*sum(x.^2)-(sum(x))^2; %old
a= (sum(x.^2)*sum(y)-(sum(x)*sum(x.*y)))/( n*sum(x.^2)-(sum(x))^2); %correct

% b= (n*sum(x.*y)-(sum(x)*sum(y)))/( n*sum(x.^2)-(sum(x))^2); %old
b= (n*sum(x.*y)-(sum(x)*sum(y)))/( n*sum(x.^2)-(sum(x))^2); %correct
